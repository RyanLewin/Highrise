...
public class Building : MonoBehaviour {
    ...
    public Transform bulldozerButton;
	...
	void LateUpdate ()
    {
        if (!driving.driving)   // if not driving...
        {
            ...
            if (!bulldozer && Input.GetKeyUp(KeyCode.B))
                BullDozer = true;
            else if (bulldozer && Input.GetKeyUp(KeyCode.B))
                BullDozer = false;
                    ...
                    if (bulldozer)
                    {
                        ...
                        if (Physics.Raycast(ray, out hit, Mathf.Infinity, layerMask))
                            if (!hit.transform.CompareTag("Untagged"))
                                if (hit.transform.GetComponent<BuildingTypes>().placed)
                                    Deletion(hit.transform);
                    }
                }
                if (toBuild && !bulldozer)
                {
        ...
        else
        {
            ...
            if (hoverSprite.gameObject.activeSelf)
    ...
    public void PlaceBuilding(Cells cell, Direction r)
    {
        ...
        if (buildPrefab.cost <= resources.money || resources.creative)
        {
            ...
            placePos = new Vector3(-cell.x * 10 - ((r == Direction.East || r == Direction.West ? size.extents.z : size.extents.x) - 5), resources.creative ? 0 : buildPrefab.buildStartY, -cell.z * 10 - ((r == Direction.East || r == Direction.West ? size.extents.x : size.extents.z) - 5));
    ...
    IEnumerator Sink (GameObject objectToDestroy)
    {
        ...
        foreach (BoxCollider coll in objectToDestroy.GetComponents<BoxCollider>())
            coll.enabled = false;
    ...
    {
        get { return bulldozer; }
        set
        {
            bulldozer = !bulldozer;
            Button btn = bulldozerButton.GetComponent<Button>();
            ColorBlock cb = btn.colors;
            if (bulldozer)
            {
                if (objectToBuild != null)
                    ResetObjectToBuild(null);
                Color col = cb.pressedColor;
                cb.normalColor = col;
                bulldozerButton.GetComponent<Image>().color = col;
                btn.colors = cb;
            }
            else
            {
                cb.normalColor = Color.black;
                bulldozerButton.GetComponent<Image>().color = Color.black;
                btn.colors = cb;
            }
        }
    }

    public void StopBulldozer ()
    {
        bulldozer = false;
        Button btn = bulldozerButton.GetComponent<Button>();
        ColorBlock cb = btn.colors;
    ...
    void Cost(int money)
    {
        ...
        if (money < 0)
            statistics.MoneyLost = -money;
        else
            statistics.MoneyMade = money;
    }
}
